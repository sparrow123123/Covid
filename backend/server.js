...
app.get("/homeslot/:id", (req, res) => {
  let sql =
    "UPDATE datalocat SET slot = slot - 1, dosage = dosage - 1 WHERE centreid=? ";
  let id = req.params.id;
  db.query(sql, [id], (err, result) => {
    if (err) throw err;
    res.send("succes");
...
app.post(
  "/homeadding/:name/:address/:location/:time/:closingtime/:pincode/:dosage",
  (req, res) => {
    let name = req.params.name;
    let address = req.params.address;
    ...
    let sql =
      "INSERT INTO datalocat(name, location,address,pincode ,optime ,cltime ,slot,dosage)values(?,?,?,67887,?,?,10,?)";
    db.query(
      sql,
      [name, location, address, time, closingtime, dosage],
      (err, result) => {
        if (err) {
          console.error(err);
        } else {
...
//////////
app.get("/newdrop/", (req, res) => {
  let sql = "select distinct loaction from datalocation;";
  db.query(sql, (err, result) => {});
  db.query(sql, (err, result) => {
    if (err) {
      console.error(err);
...
app.get("/dphelp:/addr", (req, res) => {
  let sql = "SELECT * FROM datalocation WHERE loaction = ?";
  let search = req.params.addr;
  // db.query(sql,[search],(err,result)=>{
  //     if(err){
  //         console.log("fail")
  //     }
