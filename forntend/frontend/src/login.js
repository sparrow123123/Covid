      ...
      event.preventDefault();
       ...
       .then((res)=>{
        if(res.data==='fail'){
          ...
          if((res.data[0].username==='admin')&& (res.data[0].password===12345 )){
            navigate('/adminhome');
          }
          else{
            update();
    ...
    <div className='log'>
      <div className="outlet">
        <form onSubmit={handleSubmit}>
            ...
            <input type="number" id="password" placeholder='Password' onChange={e=>setpassword(e.target.value)}></input>

            <button id='login-btn' onClick={()=>{
            
            }}  >Login</button>
            <b><p>Dont have a account?</p></b>
            <Link to="/signup"><button id='search-btn'>sign up</button> </Link>
